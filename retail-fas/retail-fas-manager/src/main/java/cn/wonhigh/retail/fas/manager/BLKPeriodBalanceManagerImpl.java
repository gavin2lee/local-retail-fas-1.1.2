...
@Service("blkPeriodBalanceManager")
public class BLKPeriodBalanceManagerImpl extends BaseCrudManagerImpl implements BLKPeriodBalanceManager {
	...
	protected BaseCrudService init() {
		return blkPeriodBalanceService;
	}

	@Override
	public BLKPeriodBalance findBLKPeriodBalanceSubTotalCount(Map<String, Object> params) throws ManagerException {
		try {
			return blkPeriodBalanceService.findBLKPeriodBalanceSubTotalCount(params);
	...
	@Override
	public List<BLKPeriodBalance> findBLKPeriodBalanceSubTotalPages(SimplePage page, String orderBy, String orderByField,Map<String, Object> params) throws ManagerException {
		try {
			return blkPeriodBalanceService.findBLKPeriodBalanceSubTotalPages(page, orderBy, orderBy, params);
		} catch (ServiceException e) {
			throw new ManagerException(e.getMessage(), e);
		}
	}
	...
	@Override
	public BLKPeriodBalance findBLKPeriodBalanceCount(Map<String, Object> params) throws ManagerException {
		try {
			return blkPeriodBalanceService.findBLKPeriodBalanceCount(params);
		} catch (ServiceException e) {
			throw new ManagerException(e.getMessage(), e);
		}
	}
	...
	@Override
	public List<BLKPeriodBalance> findBLKPeriodBalancePages(SimplePage page,String orderBy, String orderByField, Map<String, Object> params) throws ManagerException {
		try {
			return blkPeriodBalanceService.findBLKPeriodBalancePages(page, orderBy, orderByField, params);
		} catch (ServiceException e) {
			throw new ManagerException(e.getMessage(), e);
		}
	}
	...
	public List<BLKPeriodBalance> setExtendsPeriodBalanceProperties(List<BLKPeriodBalance> blkPeriodBalances) {
		...
		for(BLKPeriodBalance blkPeriodBalance:blkPeriodBalances){
			...
			if(!styleNos.contains(blkPeriodBalance.getStyleNo())){
				styleNos.add(blkPeriodBalance.getStyleNo());
			}
		}
		
    	List<BLKPeriodBalance> retList = new ArrayList<BLKPeriodBalance>();
    	Context context = new Context(this,blkPeriodBalances.get(0),itemNos,storeNos,styleNos);
    	if (!CollectionUtils.isEmpty(blkPeriodBalances)) {
    		try{
	...
	private BLKPeriodBalance setExtendsProperties(BLKPeriodBalance periodBalance, Context context) {
		...
		if (!CollectionUtils.isEmpty(items)) {
			...
			HeadquarterCostMaintain preHeadquarterCostMaintain = context.getPreHeadquarterCostMaintain(item.getItemNo());
			...
			periodBalance.setPurchaseSeason(context.getLookupEntryName(LookupEnum.SEASON.getCode(), item.getPurchaseSeason()));
			
			//设置商品标志
			CacheContext cacheContext = CacheContext.current();
			Item i = cacheContext.getItem(periodBalance.getItemCode(), periodBalance.getBrandNo());
			if(null != i){
				periodBalance.setItemFlag(i.getItemFlag());
			}
	...
	private static class Context{
		...
		public Map<String, BigDecimal> unitCosts = new HashMap<String, BigDecimal>();//款号单位成本
		...
		BLKPeriodBalanceManagerImpl parent;
		
		public Context(BLKPeriodBalanceManagerImpl parent,BLKPeriodBalance blkPeriodBalance,List<String> itemNos,List<String> storeNos,List<String> styleNos){
			this.parent = parent;
			loadCategory();
			...
			loadOrgan();
			loadBrand();
			loadUnitCost(styleNos,blkPeriodBalance);
		...
		public BigDecimal getUnitCost(String styleNo){
			return unitCosts.get(styleNo);
		}
		...
		void loadUnitCost(List<String> styleNos,BLKPeriodBalance blkPeriodBalance){
			try {
				Integer year = Integer.parseInt(blkPeriodBalance.getYear());
				Integer month = Integer.parseInt(blkPeriodBalance.getMonth());
				
				Map<String, Object> params = new HashMap<String, Object>();
				params.put("companyNo", blkPeriodBalance.getCompanyNo());
				params.put("year", year);
				params.put("month", month);
				params.put("styleNos", styleNos);
				SimplePage page = new SimplePage(1, Integer.MAX_VALUE, Integer.MAX_VALUE);
				List<ItemCost> ls = parent.itemCostService.findBLKItemCostList(page, null, null, params);
				for (ItemCost ic : ls) {
					unitCosts.put(ic.getStyleNo(), ic.getUnitCost());
				}
			} catch (ServiceException e) {
		...
		void loadPreHeadquarterCostMaintain(List<String> itemNos,BLKPeriodBalance blkPeriodBalance){
			try {
				...
				List<HeadquarterCostMaintain> ls = queryHeadquarterCostMaintain(itemNos, cal.get(Calendar.YEAR), cal.get(Calendar.MONTH) + 1);
		...
		private List<HeadquarterCostMaintain> queryHeadquarterCostMaintain(
				List<String> itemNos, Integer year, Integer month)
	...
	@Override
	public int selectStoreBanalceCount(Map<String, Object> params) throws ManagerException {
		try {
			return blkPeriodBalanceService.selectStoreBanalceCount(params);
		} catch (ServiceException e) {
			throw new ManagerException(e.getMessage(),e);
		}
	}
	...
	public List<BLKPeriodBalance> selectStoreBalanceList(SimplePage page,
		...
		try {
			return blkPeriodBalanceService.selectStoreBalanceList(page, orderBy, orderByField, params);
		} catch (ServiceException e) {
			throw new ManagerException(e.getMessage(),e);
		}
	}
	...
	@Override
	public List<BLKPeriodBalance> setExtendsProperties(List<BLKPeriodBalance> blkPeriodBalances) {
		if (CollectionUtils.isEmpty(blkPeriodBalances)) {
			return new ArrayList<BLKPeriodBalance>();
		}
    	...
    	context.search = search.open();
		for (BLKPeriodBalance pBalance : blkPeriodBalances) {
			retList.add(setStoreBalanceExtendsProperties(pBalance,context));
		}
		return retList;
	}

	private BLKPeriodBalance setStoreBalanceExtendsProperties(BLKPeriodBalance periodBalance, Context context) {
		List<cn.wonhigh.retail.mdm.common.model.Item> items = context.search.get("item", periodBalance.getItemCode());
		LOGGER.debug("获取主数据的商品缓存");
		if (!CollectionUtils.isEmpty(items)) {
			cn.wonhigh.retail.mdm.common.model.Item item = items.get(0);
			...
			OrderUnit ou = context.getOrderUnit(periodBalance.getOrderUnitNo());
			if(null != ou){
				periodBalance.setOrderUnitName(ou.getName());
			}
			
			Store store = context.getStore(periodBalance.getStoreNo());
			if(null != store){
				periodBalance.setStoreName(store.getFullName());
			}
			
			if(null != ou){
				Organ organ = context.getOrgan(ou.getOrganNo());
				if(null != organ){
					periodBalance.setOrganName(organ.getName());
				}
			}
			BigDecimal unitCost = context.getUnitCost(item.getStyleNo());
			if(null != item.getBrandNo() && item.getBrandNo().startsWith("FF")){
				unitCost = context.getUnitCost(item.getCode());
			}
			
			if(null != unitCost){
				periodBalance.setUnitCost(unitCost);
				periodBalance.setClosingBalance(unitCost.multiply(BigDecimal.valueOf(periodBalance.getClosingQty())));
			}else{
				periodBalance.setUnitCost(BigDecimal.ZERO);
				periodBalance.setClosingBalance(BigDecimal.ZERO);
			}
			
			HeadquarterCostMaintain headquarterCostMaintain = context.getHeadquarterCostMaintain(item.getItemNo());
			if(null != headquarterCostMaintain && null != headquarterCostMaintain.getHeadquarterCost()){
				periodBalance.setHeadquarterCost(headquarterCostMaintain.getHeadquarterCost());
				BigDecimal closingQty = BigDecimal.valueOf(periodBalance.getClosingQty());
				periodBalance.setClosingHeadquarterCost(closingQty.multiply(headquarterCostMaintain.getHeadquarterCost()));
			}else{
				periodBalance.setHeadquarterCost(BigDecimal.ZERO);
				periodBalance.setClosingHeadquarterCost(BigDecimal.ZERO);
			}
