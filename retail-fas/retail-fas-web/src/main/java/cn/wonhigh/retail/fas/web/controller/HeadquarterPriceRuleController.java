 ...
 */
@Controller
@RequestMapping("/headquarter_price_rule")
@ModuleVerify("30120013")
public class HeadquarterPriceRuleController extends BaseController<HeadquarterPriceRule> {
    @Resource
    private HeadquarterPriceRuleManager headquarterPriceRuleManager;

    @Override
    public CrudInfo init() {
        return new CrudInfo("headquarter_price_rule/",headquarterPriceRuleManager);
    }
	...
	public ResponseEntity<Integer> checkRuleRefered(HttpServletRequest req,Model model) throws ManagerException {
		...
		int total= headquarterPriceRuleManager.checkIsRuleRefered(params);
	...
	public ResponseEntity<Integer> getCheckUniqueCount(HttpServletRequest req,Model model)throws ManagerException{
		Map<String, Object> params = builderParams(req, model);
		int total= headquarterPriceRuleManager.findCount(params);
		return new ResponseEntity<Integer>(total, HttpStatus.OK);
	}
	...
	public Map<String, Object> addYear(HttpServletRequest req,Model model)throws ManagerException, JsonParseException, JsonMappingException, IOException{
		...
		if(StringUtils.isNotBlank(req.getParameter("checkedRows"))){
			...
			List<RegionPriceRule> lstItem = convertListWithTypeReference(mapper, itemList, HeadquarterPriceRule.class);
			Date currDate = new Date();
			String updateUser = Authorization.getUser().getUsername();
			for (HeadquarterPriceRule rule : lstItem) {	
				...
				if(StringUtils.isNotBlank(ruleYear) && ruleYear.indexOf(year) == -1){
					...
					headquarterPriceRuleManager.modifyById(rule);
				}
