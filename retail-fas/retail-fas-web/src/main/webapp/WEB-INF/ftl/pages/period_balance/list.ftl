...
<script type="text/javascript" src="${resourcesUrl}/fas/resources/js/modules/cost/periodBalance.js?version=${version}"></script>
</head>
        	...
        	<div data-options="region:'center',border:false">
		      <@p.datagrid id="dataGridDiv" loadUrl="" saveUrl="" defaultColumn=""  
		              isHasToolBar="false" onClickRowEdit="false" pagination="true" checkOnSelect="true" selectOnCheck="true"
					   ...
					   frozenColumns="[
									{title : '冻结属性',width: 120,align:'left',halign:'center',colspan:'6'},
					   			  ],
					   			  [
					   			  	{field : 'itemCode',title : '商品编码',width: 120,align:'left',halign:'center'},
				                  	{field : 'itemName',title : '商品名称',width: 200,align:'left',halign:'center'},
				                  	{field : 'brandUnitName',title : '品牌部',hidden:true,width: 80,align:'center'},
				                  	{field : 'brandName',title : '品牌',width: 80,align:'center',formatter:function(value,row,index){
										if (row.companyNo.indexOf('小计') == -1){
									...
									{field : 'balanceDiff',title : '对账差异数量',width: 100,align:'center'},
				                  	{field : 'firstLevelCategoryName',title : '一级大类',width: 80,align:'center',formatter:function(value,row,index){
										if (row.companyNo.indexOf('小计') == -1){
											return row.firstLevelCategoryName;
										}else{
											return '';
										}
									}},
			           ...
			           columnsJsonList="[
				                ...
				                [
				                  ...
				                  {field : 'preSumOweQty',title : '期初累计欠客数量',width: 120,align:'right', exportType:'number'},
				                  <!--{field : 'preSumOweAmount',title : '期初累计欠客金额',width: 100,align:'right', exportType:'number'},-->
				                  {field : 'preAccountingQty',title : '期初财务存数量',width: 120,align:'right', exportType:'number'},
				                  ...
				                  <!--{field : 'costAdjustmentAmount',title : '库存调整金额',width: 100,align:'right', exportType:'number'},-->
				                  ...
				                  <!--{field : 'salesSumAmount',title : '期间销售金额',width: 100,align:'right', exportType:'number'}-->
